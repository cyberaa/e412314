...
enumbra-core is a replicated state machine that maintains a local copy of a cryptographic ledger and processes transactions against it, in consensus with a set of peers.
It implements the [enumbra Consensus Protocol](https://github.com/enumbra/enumbra-core/blob/master/src/scp/readme.md), a _federated_ consensus protocol.
It is written in C++11 and runs on Linux, OSX and Windows.
Learn more by reading the [overview document](https://github.com/enumbra/enumbra-core/blob/master/docs/readme.md).

# Documentation

Documentation of the code's layout and abstractions, as well as for the
functionality available, can be found in
[`./docs`](https://github.com/enumbra/enumbra-core/tree/master/docs).
...
Software has bugs, or maybe you have an idea for a change in enumbra-core.
...
run tests with:
  `src/enumbra-core --test`

run one test with:
  `src/enumbra-core --test  testName`

run one test category with:
  `src/enumbra-core --test '[categoryName]'`
...
supported test options can be seen with
  `src/enumbra-core --test --help`

display tests timing information:
  `src/enumbra-core --test -d yes '[categoryName]'`

xml test output (includes nested section information):
  `src/enumbra-core --test -r xml '[categoryName]'`
...
Then, running
* `enumbra-core --test [stress]` will run all the stress tests,
* `enumbra-core --test [foo-stress]` will run the stress tests for subsystem foo alone, and
* neither `enumbra-core --test` nor `enumbra-core --test [foo]` will run stress tests.
